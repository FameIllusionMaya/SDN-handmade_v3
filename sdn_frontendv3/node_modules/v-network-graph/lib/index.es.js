...
function provideMouseOperation(container, nodePositions, zoomLevel, nodeStates, edgeStates, selectedNodes, selectedEdges, hoveredNodes, hoveredEdges, emitter) {
  ...
  function handleEdgePointerUpEvent(event) {
    ...
    edges.forEach((edge2) => {
      emitter.emit("edge:pointerup", { edge: edge2, event, summarized: edges.length > 1 });
    });
  ...
  function handleEdgeClickEvent(edge, event) {
    ...
    selectedNodes.clear();
    const edges = edge instanceof Array ? edge : [edge];
    const isTouchAnySelectedEdge = MapUtil.valueOf(state.edgePointers).filter((p) => {
      const edges2 = p.edgeId instanceof Array ? p.edgeId : [p.edgeId];
      return p.pointerId != event.pointerId && edges2.every((edge2) => selectedEdges.has(edge2));
    }).length > 0;
    ...
    edges.forEach((edge2) => {
      emitter.emit("edge:click", { edge: edge2, event, summarized: edges.length > 1 });
    });
  }
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("g", {
        ...
        createVNode(VLine, mergeProps(pos.value, {
          ...
          onPointerdown: _cache[0] || (_cache[0] = withModifiers(($event) => unref(handleEdgesPointerDownEvent)(unref(edgeIds), $event), ["prevent", "stop"])),
...
var VEdgeLabels = /* @__PURE__ */ _export_sfc(_sfc_main$a, [["__scopeId", "data-v-15f17dbf"]]);
...
var networkGraph = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["render", _sfc_render], ["__scopeId", "data-v-7a086c44"]]);
